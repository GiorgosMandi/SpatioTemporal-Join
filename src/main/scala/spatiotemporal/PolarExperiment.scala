  ...
  def nextOption(map: OptionMap, list: List[String]): OptionMap = {
   ...
   def main(args: Array[String]): Unit = {
      ...
      var observationRDD =
        if (options.contains("ob")) {
          ...
          ob
            ...
            .map(line => ObservationRecord(line))
            .map(rec => (STObject(rec.point,  new Instant(rec.observed_at)), rec))
        }
        else {
        ...
        var sentinelImagesRDD = if (options.contains("s1")) {
          ...
          s1
            .filter(line => ! line.sameElements(header))
            .map(line => SentinelImageRecord(line))
            .map(rec => (STObject(rec.coverage,  new Instant(rec.datetime)), rec))
        }
        else {
     ...
     val dfS1 = spark.createDataFrame(
       joinedRDD
         .map(rec => Row(rec._1.id, rec._1.datetimeSTR, rec._1.coverage))
       ,schemaS1
     ).distinct()

     val dfObs = spark.createDataFrame(
       joinedRDD
         .map(rec => Row(rec._1.id, rec._2.id, rec._2.observed_at_STR, rec._2.point))
       ,schemaObs
     )
